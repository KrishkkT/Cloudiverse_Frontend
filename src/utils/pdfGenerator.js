...
export const generateProjectReport = async (project, diagramImage) => {
    ...
    const addFooter = (pageNumber) => {
        ...
        const linkText = "www.cloudiverse.app";
    ...
    // COST & ENV LOGIC FIX
    let displayedEnv = infraSpec.environment || 'Production';
    let costValue = 0;

    // Parse cost safely
    if (costData.recommended && costData.recommended.totalMonthlyCost) {
        costValue = parseFloat(costData.recommended.totalMonthlyCost);
    } else if (costData.totalMonthlyCost) {
        costValue = parseFloat(costData.totalMonthlyCost.replace(/[^0-9.]/g, ''));
    }

    // Logic: If Production but $0 cost, force downgrade Env status to maintain credibility
    if (displayedEnv === 'Production' && costValue === 0) {
        displayedEnv = 'Development / Preview (Ref Warning)';
    }
    ...
    doc.setFontSize(10);
    doc.setTextColor(180, 100, 0); // Dark Amber
    doc.setFont("helvetica", "bold");
    doc.text("⚠ Deployment Note: Conceptual Reference", margin + 5, yPos + 8);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(60, 60, 60);
    const archDisclaimer = "This design is high-level. Actual deployment will generate provider-specific resources (e.g., IAM Roles, Security Groups, VPCs) that are abstracted here for clarity.";
    const archLines = doc.splitTextToSize(archDisclaimer, pageWidth - (margin * 2) - 10);
    doc.text(archLines, margin + 5, yPos + 16);
    yPos += 35;
    ...
    if (costValue === 0) {
        ...
        doc.text("Incomplete Selection", margin + 10, yPos + 30);
        doc.setFontSize(10);
        doc.text("(Pricing metadata unavailable for selected services)", margin + 80, yPos + 30);
    } else {
    ...
    if (services.length > 0) {
        const body = services.map(s => {
            let sName = s.name || s.service || 'Unknown';
            let sCost = s.cost || s.price || '-';
            let sUnit = s.unit || 'Monthly';

            // HANDLE UNKNOWN SERVICE / NULL COST
            if (sName === 'Unknown' || sCost === '-' || sCost === '0' || sCost === 0) {
                // Formatting for unpriced services
                sCost = "Manual Est.";
                sUnit = "Not in Infracost";
            }

            return [formatServiceName(sName), sCost, sUnit];
        });
    ...
    // --- PAGE 4: DEPLOYMENT SPECIFICATIONS ---
    doc.addPage();
    addHeader("Deployment Specifications");
    yPos = 40;

    addSectionTitle("Operational Assumptions");
    const assumptions = [
        ['Code Repository', 'GitHub (Recommended)'],
        ['Containerization', 'Docker / OCI Standards'],
        ['State Management', 'Terraform State (Remote Backend)'],
        ['Ownership', 'User-Managed (Self-Hosted Model)']
    ];
